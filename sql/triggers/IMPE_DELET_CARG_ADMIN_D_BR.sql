...
begin
    ...
    if xADMIN = 1 and old.CARGO_ID != new.CARGO_ID then
        SIGNAL SQLSTATE '45000' set MESSAGE_TEXT = '|O usuário admin não pode ter seu cargo alterado';
    end if;
...
DELIMITER $$
create trigger IMPE_UPDT_PERM_USU_ADMIN_D_BR
before update on FUNCIONARIOS
for each row
begin
    ...
    if xADMIN = 1 and old.PERMISSAO_ID != new.PERMISSAO_ID then
        SIGNAL SQLSTATE '45000' set MESSAGE_TEXT = '|O usuário admin não pode ter sua permissão alterada';
    end if;
